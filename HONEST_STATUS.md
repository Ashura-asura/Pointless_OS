...
| 10 — Supervision hardening + chaos + formal ceiling | 65% | Ceiling property tests (14), model chaos tests (6), model supervision (10), **real-kernel chaos harness (`chaos.rs`, feature-gated, 3 host tests), TLA+ ceiling proof model-checked (AegisCeiling.tla: 5.64M states / 147k distinct / 0 errors)** | Ceiling is a finite-instance model-check, not an inductive proof; live chaos iteration-count numbers still need a real boot |
...
| 10 | Supervision/circuit-breaker hardening + chaos testing | ✅ Done: supervision-tree (4) + chaos (6) model tests; adaptive-ceiling verification (14) in aegis-kernel — caught+fixed real scope-expansion bug in tighten_scope. **Phase L (this commit): the chaos harness is now REAL code against the kernel's real Supervisor/Fleet/frame-allocator** (`aegis-kernel/src/chaos.rs`, feature-gated `chaos-demo`, 3 host-testable tests incl. a 500-seed network-partition fail-closed sweep; task-crash + resource-exhaustion scenarios execute live at boot via `IDX_SERVICE`) — plus **a TLA+ ceiling proof (`aegis/spec/AegisCeiling.tla`/.cfg) that is ACTUALLY model-checked, not just drafted**: `CeilingInvariant` (no role's effective scope ever exceeds its granted scope on any dimension) and `QueryAdvisorNeverEscapesHostScope` (the advisor's host scope never becomes `AnyHost`) hold across TLC's full search — **5,644,801 states generated, 147,456 distinct, 0 errors** — with a negative control (wiring `WidenAttempt` into `Next`) that TLC finds violating `InvAll` at depth 2, proving the invariant has teeth. Honest ceilings: chaos is iteration-count (not a fabricated wall-clock duration) and the task-crash/resource-exhaustion scenarios need a real boot to produce numbers; the TLA+ ceiling is a finite-instance model-check (3 roles, scope dims bounded), not an inductive seL4-class proof |
