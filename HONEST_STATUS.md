...
- **Full 12-phase roadmap completion: ~60%** (unweighted mean of the per-phase
  ...
  fencing, real GPU, hypervisor vehicles).
...
| 3 — IOMMU-backed drivers (NVMe/NIC/GPU) | 50% | Real NVMe driver (live QEMU); real e1000e NIC driver (live ARP TX/RX under QEMU, externally captured); **real TCP/IP over the NIC under QEMU** (handshake + HTTP request/response + close, externally captured) | `iommu.rs` is a stub ("can't do MMIO here"); NIC driver is polled (no interrupts/MSI-X), boot-time demo only, not IOMMU-fenced; no GPU path; drivers not IOMMU-fenced |
...
| 5 — Networking as a service | 85% | Real capability-gated loopback `netstack.rs` (6 tests); real e1000e NIC driver (7 tests); **a full polled TCP/IP stack in the kernel (`netif.rs`, 9 tests) that drives a real three-way handshake, HTTP request/response and close over the wire under QEMU** — SYN→SYN-ACK→ACK→GET→HTTP response→FIN captured externally on the host (pcap), with the HTTP response body rendered by the kernel; **plus a real TLS 1.3 ClientHello/ServerHello exchange on a second socket (`tls.rs`, 8 tests): the kernel sends a spec-correct ClientHello to an OpenSSL-backed server on 8443, parses the ServerHello, and derives the X25519 ECDHE shared secret that matches the host byte-for-byte, externally captured** | No interrupts (polled only); QEMU socket netdev, not physical hardware; TLS handshake not yet completed (no AES-GCM / Finished / cert verify — Phase E remainder); no outbound routing/NAT; single connection demo wired in `main.rs`, syscalls 19–23 provide the capability-scoped socket API for tasks |
...
| Polled TCP/IP stack (`netif.rs`) | 9 | Client stack on the e1000e driver: ARP resolve (table/request/reply), IPv4 TX (checksums), RX demux ARP/ICMP/UDP/TCP, ICMP echo reply, TCP client state machine (Closed/SynSent/Established/FinWait1/FinWait2) with seq/ack handling, RTO retransmission, slow-start cwnd, send-buffer compaction, UDP send/recv, and the capability-scoped socket API (syscalls 19–23, `Cap::NetEndpoint`, SEND|RECV gated). **LIVE under QEMU**: real three-way handshake, HTTP request/response, and FIN close against an external host peer, externally captured (see Phase-E row). Honest limits: client-only (no LISTEN/accept), polled clock for RTO, single destination per socket, TLS handshake not yet completed (no AES-GCM/Finished — see `tls.rs`, 8 tests) |
| TLS 1.3 client (`tls.rs`) | 8 | Vec-free record layer (5-byte header, content types 22/23/24); `build_client_hello` (server_name + supported_versions 0x0304 + signature_algorithms + supported_groups 0x001d + key_share x25519, layouts validated against a real OpenSSL client); `parse_server_hello` (KeyShareEntry u16 `key_exchange` length); X25519 scalar multiplication (RFC 7748 §5.2 vectors 1 & 2 + base-point-all-zero rejection); HKDF-Extract/Expand/ExpandLabel (RFC 5869 case 1). **LIVE under QEMU**: ClientHello → OpenSSL ServerHello, ECDHE shared secret cross-checked byte-for-byte with the host peer (see Phase-E row). Honest limits: handshake not yet completed (no AES-128-GCM / Finished / certificate verify); fixed deterministic scalar (no CSPRNG in the guest) |
...
| Real network I/O on a NIC | Real e1000e driver + live ARP TX/RX (7 tests) + live TCP/IP (9 tests) | Real Ethernet frames leave and re-enter the kernel under QEMU (polled e1000e driver; external host capture + ARP echo). **Phase E: the kernel's `netif.rs` stack drives a real TCP connection over that NIC** — ARP resolve, SYN→SYN-ACK→ACK handshake, an HTTP request/response, and a FIN close, all externally captured in `e1000-phase-e.pcap` with the HTTP response body rendered by the kernel. Honest limits: polled (no interrupts); QEMU socket netdev, not physical hardware; client-only (no LISTEN/accept); TLS handshake not yet completed (no AES-GCM/Finished) |
