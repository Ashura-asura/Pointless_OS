...
| 3 — IOMMU-backed drivers (NVMe/NIC/GPU) | 40% | Real NVMe driver (live QEMU); real e1000e NIC driver (live ARP TX/RX under QEMU, externally captured) | `iommu.rs` is a stub ("can't do MMIO here"); NIC driver is polled (no interrupts/MSI-X), boot-time demo only, not IOMMU-fenced; no GPU path; drivers not IOMMU-fenced |
...
| 5 — Networking as a service | 60% | Real capability-gated loopback `netstack.rs` (6 tests); real e1000e NIC driver (7 tests) — polled TX/RX with a live ARP request/reply exchange under QEMU, the guest's frame captured externally on the host (pcap) | NIC is a boot-time demo, NOT wired into the socket layer; polled only (no interrupts); UDP/TCP header models still separate (`ethernet/arp/ipv4/udp/tcp` 43 tests); QEMU socket netdev, not real hardware |
...
| Real network I/O on a NIC | Real e1000e driver + live ARP TX/RX (7 tests) | Real Ethernet frames now leave and re-enter the kernel under QEMU (polled e1000e driver; external host capture + ARP echo). Honest limits: boot-time demo only — NOT wired into the socket layer; polled (no interrupts); QEMU socket netdev, not physical hardware; no IP routing/UDP/TCP; UDP/TCP remain header parse/serialize models only |
