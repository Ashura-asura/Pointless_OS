...
| 5. Anomaly circuit-breaking + two-party | **REDUCED (do more later)** | Two-party IS closed for the high-risk (WRITE/"apply") expansion: a distinct second reviewer is required, same-party-twice refused (`apply_edit_requires_two_party_confirmation`). The *anomaly* half is reduced to a lightweight, role-shape check (`reader_shape_ok`) reusing the same attributed audit log `monitor.rs` trains on — it flags a read-only reader that exercised a mutating op. The full suspend-don't-revoke monitor (`monitor.rs`) is not yet wired to role grants; that is the honest remaining gap. |
