...
// xorshift64, same algorithm as chaos.rs's Rng (independently reimplemented
// here since this is a standalone crate, not a dependency on the kernel).
struct Rng(u64);
impl Rng {
...
/// because most random buffers get rejected in the first few bytes
/// (wrong magic) and never reach the interesting bounds-checking logic
/// deeper in each parser.
...
impl Report {
    ...
    fn print(&self) {
        println!(
            "[{}] ran={} panics={}",
            self.label, self.ran, self.panics
        );
...
fn run_target(label: &'static str, iterations: u64, max_len: usize, seed: u64, seeds: &[Vec<u8>], f: impl Fn(&[u8])) -> Report {
    let mut rng = Rng::new(seed);
    let mut report = Report::new(label);

    // Phase 1: pure random bytes, every length from 0..=max_len equally
    // reachable.
    ...
    // Phase 2: mutation-based, starting from real valid seeds — this is
    // what actually exercises the bounds-checking logic deep in each
    // parser instead of bouncing off the magic-number check every time.
...
fn main() {
    ...
    println!("Phase M fuzz campaign — seed={:#x} iterations_per_target={}", seed, iterations);
    ...
    let r1 = run_target("decode_entries", iterations, 512, seed ^ 1, &entries_seeds, |data| {
        let mut out = [store_decode::FileEntry::default(); store_decode::MAX_FILES];
        let _ = store_decode::decode_entries(data, &mut out);
    });

    let r2 = run_target("parse_elf", iterations, 1024, seed ^ 2, &elf_seeds, |data| {
        let _ = elf_loader::parse_elf(data);
    });

    let r3 = run_target("parse_pe", iterations, 1024, seed ^ 3, &pe_seeds, |data| {
        let _ = pe_loader::parse_pe(data);
    });
    ...
    if total_panics == 0 {
        ...
        println!("RESULT: {} crash(es) found — see first-panic inputs above", total_panics);
        std::process::exit(1);
    }
}
