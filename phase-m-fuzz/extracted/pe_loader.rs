...
fn opt_is_none_or<T>(o: Option<T>, f: impl FnOnce(T) -> bool) -> bool {
    match o { None => true, Some(x) => f(x) }
}
...
pub fn parse_pe(data: &[u8]) -> Result<PeImage, &'static str> {
    ...
    if opt_is_none_or(pe_off.checked_add(24), |end| end > data.len()) {
    ...
    if opt_is_none_or(opt_off.checked_add(64), |end| end > data.len()) {
    ...
    let section_bytes = num_sections
        .checked_mul(40)
        .ok_or("section table offset overflow")?;
    if opt_is_none_or(section_table.checked_add(section_bytes), |end| end > data.len())
    {
